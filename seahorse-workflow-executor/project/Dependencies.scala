...
object Version {
  ...
  val (scala, java, hadoop, akka, apacheCommons) = spark match {
    ...
    case "2.0.0" | "2.0.1" | "2.0.2" => ("2.12.6", "1.8", "2.7.1", "2.4.12", "3.3.+")
...
object Library {
  ...
  implicit class RichModuleID(m: ModuleID) {
    ...
    def excludeJackson = m excludeAll (
  ...
  // Fixed spark function to apply both exclusions correctly
  val spark = (name: String) => (version: String) => "org.apache.spark" %% s"spark-$name" % version excludeAll (
  ...
  val wireMock = "com.github.tomakehurst" % "wiremock" % Version.wireMock exclude ("com.google.guava", "guava") excludeJackson
